    ...
    try:
        ...
        if request.hintGiven == 0:
            ...
            if len(request.quiz) == 2:
                answer_list = Words.objects.filter(word_length=len(request.quiz), first_sound=request.quiz[0])
            else:
                answer_list = Words.objects.filter(word_length=len(request.quiz), first_sound=request.quiz[0], last_sound=request.quiz[-1])
        #힌트를 준 뒤라면, 
        if not len(answer_list):
            #틀린 경우
